...
const ASSET_BASE = './assets/rougelike/';
...
class Enemy {
  constructor(def, floor) {
    ...
    const depthBonus = Math.max(0, floor - 1);
    this.maxHp = Math.floor(def.hp * scale);
    this.hp = this.maxHp;
    this.atk = Math.floor(def.atk * scale + depthBonus * 0.8);
    this.def = Math.floor(def.def * scale + depthBonus * 0.35);
  ...
  _buildDropTable(id, floor) {
    const drops = [];
    // ポーション
    if (chance(0.22)) drops.push(chance(0.55) ? 'hp_potion' : 'mp_potion');
    // 装備 (floor >= 3 から確率増加)
    if (floor >= 3 && chance(0.1 + floor * 0.005)) drops.push(pick(['sword', 'spear', 'bow', 'staff', 'armor', 'shield', 'helmet']));
    // スクロール
    ...
    if (chance(0.16)) drops.push(pick(scrolls));
    // お宝
    if (chance(0.08 + floor * 0.012)) drops.push('treasure');
    return drops;
  }
...
class CombatResolver {
  ...
  static calcDamage(atkStr, defVit, atkSide_STR, def_VIT) {
    const base = Math.max(1, atkStr * 0.72 - defVit * 0.42);
    const mod = clamp(1 + (atkSide_STR - def_VIT) * 0.06, 0.5, 1.8);
...
class DungeonFloor {
  ...
  _generate() {
    ...
    for (let i = 1; i < rooms.length - 1; i++) {
      ...
      const monsterCount = rnd(1, 2) + (this.floorNum >= 4 && chance(0.55) ? 1 : 0);
      ...
      // 罠
      if (chance(0.28 + this.floorNum * 0.015)) {
...
class Dungeon3D {
  constructor(canvas) {
    ...
    this._monsterTexture = this._textureLoader.load(`${ASSET_BASE}monsters.png`);
    this._monsterTexture.colorSpace = THREE.SRGBColorSpace;
    this._propTextures = {
      torch: this._loadPropTexture('torch.png'),
      chest: this._loadPropTexture('treasure-chest.png'),
      stairsDown: this._loadPropTexture('stairs-down.png'),
      trap: this._loadPropTexture('trap.png'),
    };
  ...
  _buildMaterials() {
    const wallMap = this._makeStoneTexture(256, '#514c40', '#2f2f2d', 22);
    const floorMap = this._makeStoneTexture(256, '#3e3b34', '#242422', 18);
    wallMap.repeat.set(1, 2);
    wallMap.wrapS = wallMap.wrapT = THREE.RepeatWrapping;
    floorMap.repeat.set(2, 2);
    floorMap.wrapS = floorMap.wrapT = THREE.RepeatWrapping;
    return {
      floor: new THREE.MeshStandardMaterial({ map: floorMap, color: 0x777066, roughness: 0.9 }),
      wall: new THREE.MeshStandardMaterial({ map: wallMap, color: 0x8a8275, roughness: 0.95 }),
      stair: new THREE.MeshStandardMaterial({ map: wallMap, color: 0x6f6a60, roughness: 0.95 }),
      chestWood: new THREE.MeshStandardMaterial({ color: 0x5a3218, roughness: 0.75 }),
      chestMetal: new THREE.MeshStandardMaterial({ color: 0xb18a43, metalness: 0.5, roughness: 0.35 }),
      ...
      // 松明素材
      torchPole: new THREE.MeshStandardMaterial({ color: 0x5b3420, roughness: 0.7 }),
      torchMetal: new THREE.MeshStandardMaterial({ color: 0x2f2924, metalness: 0.45, roughness: 0.45 }),
      torchFlame: new THREE.MeshStandardMaterial({ color: 0xff9a22, emissive: new THREE.Color(0xff5500), emissiveIntensity: 1.7 }),
    };
  }

  _loadPropTexture(fileName) {
    const texture = this._textureLoader.load(`${ASSET_BASE}${fileName}`);
    texture.colorSpace = THREE.SRGBColorSpace;
    return texture;
  }

  _makeStoneTexture(size, baseColor, darkColor, blockSize) {
    ...
    for (let y = 0; y < size; y += blockSize) {
      const offset = (Math.floor(y / blockSize) % 2) * Math.floor(blockSize / 2);
      for (let x = -offset; x < size; x += blockSize) {
        const w = blockSize + rnd(-4, 5);
        const h = blockSize + rnd(-4, 5);
        const shade = rnd(-24, 24);
        ctx.fillStyle = this._shadeColor(baseColor, shade);
        ctx.fillRect(x + 1, y + 1, w - 2, h - 2);
        ctx.strokeStyle = 'rgba(0,0,0,0.42)';
        ctx.strokeRect(x + 0.5, y + 0.5, w, h);
        ctx.fillStyle = 'rgba(255,255,255,0.08)';
        ctx.fillRect(x + 3, y + 3, Math.max(1, w - 8), 2);
      }
    }
  ...
  _monsterMaterial(id) {
    const idx = MONSTER_SPRITE_INDEX[id] ?? 1;
    const tex = this._monsterTexture.clone();
    tex.repeat.set(0.25, 0.25);
    tex.offset.set((idx % 4) * 0.25, 1 - (Math.floor(idx / 4) + 1) * 0.25);
    tex.needsUpdate = true;
    return new THREE.SpriteMaterial({ map: tex, transparent: true, alphaTest: 0.08, depthWrite: false });
  }

  _createPropSprite(texture, width, height, y, additive = false) {
  ...
  _createFloorDecal(texture, size) {
  ...
  buildFloor(dungeonFloor, px, py) {
    ...
    const stairStepGeo = new THREE.BoxGeometry(0.85, 0.08, 0.18);
      ...
      for (let x = 0; x < floor.W; x++) {
        ...
        if (t === TILE.WALL) {
          ...
          for (let i = 0; i < steps; i++) {
            const sm = new THREE.Mesh(stairStepGeo, this._materials.stair);
            sm.position.set(wx, 0.04 + i * 0.07, wz - 0.32 + i * 0.15);
            sm.receiveShadow = true;
            this._meshGroup.add(sm);
          }
          const stairSprite = this._createPropSprite(this._propTextures.stairsDown, 1.25, 0.85, 0.48);
          stairSprite.position.x = wx;
          stairSprite.position.z = wz - 0.05;
          this._meshGroup.add(stairSprite);
        } else if (t === TILE.CHEST) {
          const chest = this._createPropSprite(this._propTextures.chest, 0.9, 0.78, 0.42);
          chest.position.x = wx;
          chest.position.z = wz;
          this._meshGroup.add(chest);
        } else if (t === TILE.DOOR) {
    ...
    for (const { x, y, wallDir } of torchPositions) {
      const wx = x - px, wz = y - py;
      const torch = this._createPropSprite(this._propTextures.torch, 0.45, 1.0, 1.05, true);
      torch.position.x = wx + wallDir[0] * 0.36;
      torch.position.z = wz + wallDir[1] * 0.36;
      this._meshGroup.add(torch);
    ...
    for (const it of floor.items) {
      ...
      let im;
      if (it.itemId === 'treasure') {
        if (floor.tiles[it.y]?.[it.x] === TILE.CHEST) continue;
        im = this._createPropSprite(this._propTextures.chest, 0.72, 0.62, 0.34);
      } else {
        im = new THREE.Mesh(new THREE.SphereGeometry(0.15, 8, 8), this._materials.item);
        im.position.y = 0.15;
      }
      im.position.x = it.x - px;
      im.position.z = it.y - py;
    ...
    for (const ent of floor.enemies) {
      ...
      const em = new THREE.Sprite(this._monsterMaterial(ent.enemy.id));
      const bossScale = ent.enemy.level >= 10 ? 1.45 : 1;
      em.scale.set(0.9 * bossScale, 1.15 * bossScale, 1);
      em.position.set(ent.x - px, 0.72 * bossScale, ent.y - py);
    ...
    for (const tr of floor.traps) {
      ...
      const tm = this._createFloorDecal(this._propTextures.trap, 0.78);
      tm.position.x = tr.x - px;
      tm.position.z = tr.y - py;
